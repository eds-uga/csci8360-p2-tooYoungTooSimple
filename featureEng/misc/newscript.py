...
if __name__ == '__main__':
    ...
    prc = preprocessor()
    urlTrainX = 'https://s3.amazonaws.com/eds-uga-csci8360/data/project2/labels/X_train_small.txt' 
    trainX = csv.reader(urllib2.urlopen(urlTrainX))
    trainXList = list([x[0] for x in list(trainX)])[0:2]
    #print prc.stripFileNames(trainXList[0])
    
    urlTrainY = 'https://s3.amazonaws.com/eds-uga-csci8360/data/project2/labels/y_train_small.txt' 
    ...
    #sc = SparkContext()
    binaryPath = "s3://eds-uga-csci8360/data/project2/binaries/"
    byteFiles = sc.wholeTextFiles(binaryPath).filter(lambda x: prc.stripFileNames(x[0]) in trainXList) 
    # feature rdd
   # prc = preprocessor()
    byteFeatureRdd = prc.byteFeatureGenerator(byteFiles, labelRdd)

    # save rddd
    print byteFeatureRdd.first()
